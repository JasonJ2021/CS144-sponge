...
StreamReassembler::StreamReassembler(const size_t capacity)
    : _output(capacity), _capacity(capacity), _buffer(capacity, '\0'), _bitmap(capacity, false) {}
...
void StreamReassembler::push_substring(const string &data, const uint64_t index, const bool eof) {
    ...
    if (index < index_unass) {
        ...
        size_t bytes_actual_written =
            std::min(data.size() - offset, _capacity - _output.buffer_size() - _unass_bytes);  // PROBLEM
